...
-- RBL's collections cycle bucket (1/2/3), on the Bucket books only. Safe to re-run.
-- Deliberately nullable and left NULL rather than '': NULL means the book carried no
-- Bucket column, '' means it carried one and this account was blank. The report needs
-- to tell those apart, and a rebuild from stored rows has to preserve the difference.
ALTER TABLE account_rows ADD COLUMN IF NOT EXISTS cycle_bucket text;
